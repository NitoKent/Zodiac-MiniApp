...
export function ZodiacList() {
  ...
  return (
    <div className='flex-grow mt-9 bg-[#fcfcfc] rounded-t-[46px] relative top-glow z-0'>
      <div className='absolute top-[3px] left-0 right-0 bottom-0 bg-[#111418] rounded-t-[44px] p-4'>
        <div className='grid grid-cols-3 gap-4 mt-5'>
          {zodiacSigns.map((sign) => (
            ...
            >
              <h1 className='text-xl text-orange-300'>{sign.name}</h1>
              <p className='text-sm'>{sign.period}</p>
              <img src={sign.img} alt={sign.name} className=' mt-5 w-24  mb-2' />
            </div>
          ))}
        </div>
      </div>
